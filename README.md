...
```
python3 fetch_spacex_last_launch.py --launch 'launch_id' --path 'dir path'
``` 
You can use script without launch id and path:
```
python3 fetch_spacex_last_launch.py
```
In that case, script will run with lastest launch and default path dir 'cd/image/'. 
...
```
python3 epic_nasa.py --token 'token' --path 'path'
```
...
```
python3 apod_nasa.py --token 'token' --path 'path' --count int(count)
```
...
```
python3 tg_bot_post_image.py --path 'path'
```
